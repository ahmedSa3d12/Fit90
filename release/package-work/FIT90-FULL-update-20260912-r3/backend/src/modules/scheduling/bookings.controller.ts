...
import { CreateNutritionBookingDto, CreatePersonalTrainingBookingDto, NutritionEligibilityQueryDto, PersonalTrainingEligibilityQueryDto, RestoreNutritionEntitlementDto, UpdateNutritionBookingStatusDto } from './dto/nutrition-booking.dto';
...
export class BookingsController {
  ...
  @Get('spa/eligibility')
  spaEligibility(@Query() query: NutritionEligibilityQueryDto) {
    return this.nutritionBookings.eligibility(query.memberId, query.serviceId, query.date, 'spa');
  }

  @Get('personal-training/eligibility')
  personalTrainingEligibility(@Query() query: PersonalTrainingEligibilityQueryDto) {
    return this.nutritionBookings.personalTrainingEligibility(query.memberId, query.date);
  }
  ...
  @Post('spa')
  @RequiresPermission('club.fitness:create')
  createSpa(@Body() body: CreateNutritionBookingDto, @CurrentUser() user: JwtUser) {
    return this.nutritionBookings.create(body, user, 'spa');
  }

  @Post('personal-training')
  @RequiresPermission('club.fitness:create')
  createPersonalTraining(@Body() body: CreatePersonalTrainingBookingDto, @CurrentUser() user: JwtUser) {
    return this.nutritionBookings.createPersonalTrainingAttendance(body, user);
  }
  ...
  @Patch('spa/:id/status')
  @RequiresPermission('club.fitness:update')
  transitionSpaStatus(@Param('id', ParseIntPipe) id: number, @Body() body: UpdateNutritionBookingStatusDto, @CurrentUser() user: JwtUser) {
    return this.nutritionBookings.transitionStatus(id, body.status, user, 'spa');
  }

  @Patch('personal-training/:id/status')
  @RequiresPermission('club.fitness:update')
  transitionPersonalTrainingStatus(@Param('id', ParseIntPipe) id: number, @Body() body: UpdateNutritionBookingStatusDto, @CurrentUser() user: JwtUser) {
    return this.nutritionBookings.transitionStatus(id, body.status, user, 'personal_training');
  }
