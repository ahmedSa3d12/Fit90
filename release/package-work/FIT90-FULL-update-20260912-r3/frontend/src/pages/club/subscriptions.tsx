...
export function ClubSubscriptionsPage({
  ...
  // Arriving from the member form's "Save and add subscription": preselect that member and open the dialog.
  useEffect(() => {
  ...
  const openFreeze = (id: number) => {
    ...
    setFreezeForm({ startDate: today, endDate: today, reason: '' });
    setFreezeOpen(true);
  };
  ...
  const submitFreeze = async () => {
    ...
    if (!freezeForm.startDate || !freezeForm.endDate) {
      toast.error(ct('subscriptions.freezeDatesRequired'));
      return;
    }
    if (freezeForm.endDate < freezeForm.startDate) {
      toast.error(ct('subscriptions.freezeEndBeforeStart'));
      return;
    }
    setSaving(true);
    try {
  ...
  const subColumns = useMemo<ColumnDef<ClubSubscriptionListItem>[]>(
    () => [
      ...
      { accessorKey: 'customerName', header: ct('subscriptions.customerName'), cell: ({ getValue }) => getValue() ?? '—' },
      {
        ...
        cell: ({ getValue }) => {
          const memberCode = getValue() as string | null;
          return memberCode ? <span className="nums" dir="ltr">{memberCode}</span> : '—';
        },
      },
      {
    ...
    <div className="space-y-6">
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as ClubSubscriptionsView)}>
        ...
        <TabsList className="flex h-auto flex-wrap gap-1">
          ...
          <TabsTrigger value="outstanding"><Wallet className="size-4" /> {ct('subscriptions.outstandingReport')}</TabsTrigger>
        ...
        <TabsContent value="outstanding" className="space-y-4 pt-4">
          <h3 className="font-medium">{ct('subscriptions.outstandingReport')}</h3>
          <DataTable
            columns={[
        ...
        <TabsContent value="reports" className="space-y-6 pt-4">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
            ...
            <div className="grid gap-2">
              ...
              >
                <option value="all">{ct('common.all')}</option>
        ...
        <DialogContent size="sm" aria-describedby={undefined}>
          ...
          <div className="grid gap-3">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="grid gap-2">
                <Label htmlFor="freeze-start-date">{ct('subscriptions.freezeStartDate')}</Label>
                <Input
                  id="freeze-start-date"
                  type="date"
                  min={localToday()}
                  value={freezeForm.startDate}
                  onChange={(e) => setFreezeForm((f) => ({
                    ...f,
              ...
              <div className="grid gap-2">
                <Label htmlFor="freeze-end-date">{ct('subscriptions.freezeEndDate')}</Label>
                <Input
                  id="freeze-end-date"
                  type="date"
                  min={freezeForm.startDate || localToday()}
            ...
            <div className="grid gap-2">
              <Label htmlFor="freeze-reason">{ct('subscriptions.freezeReason')}</Label>
              <Input
                id="freeze-reason"
