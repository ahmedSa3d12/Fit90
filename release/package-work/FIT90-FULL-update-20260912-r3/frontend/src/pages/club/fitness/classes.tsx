...
import type { ClubClassDetail, ClubClassRow, ClubClassStatistics, ClubTrainerRow } from '@/types/fitness';
...
export function FitnessClassesPage({ personalOnly = false }: FitnessClassesPageProps = {}) {
  ...
  const filters: FilterField[] = [
    ...
    {
      key: 'classType',
      label: ft('classes.classType'),
      type: 'select',
      options: [
        { value: 'class', label: ft('classes.typeClass') },
        { value: 'zumba', label: ft('classes.typeZumba') },
        { value: 'aerobics', label: ft('classes.typeAerobics') },
        { value: 'yoga', label: ft('classes.typeYoga') },
        { value: 'crossfit', label: ft('classes.typeCrossfit') },
      ],
    },
  ];
  ...
  const [enrollMember, setEnrollMember] = useState<ClubMemberListItem | null>(null);
  ...
  const enroll = async (waitlistIfFull = false) => {
    if (!enrollClassId || !enrollMember) {
      toast.error(ft('sched.pickMember'));
      return;
    }
    setSaving(true);
    try {
      await api.post(`/club-classes/${enrollClassId}/enroll/${enrollMember.id}`, null, {
  ...
  const columns = useMemo<ColumnDef<ClubClassRow>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="nums">
            {toArabicDigits(row.original.startTime)} – {toArabicDigits(row.original.endTime)}
          </span>
        ),
      },
      {
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg" aria-describedby={undefined}>
          ...
          <div className="grid gap-3 py-2">
            ...
            <div className="grid gap-2">
              <Label>{ft('classes.classType')}</Label>
              <select className={SELECT_CLS} value={form.classType} onChange={(e) => setForm((f) => ({ ...f, classType: e.target.value }))}>
                <option value="class">{ft('classes.typeClass')}</option>
                <option value="zumba">{ft('classes.typeZumba')}</option>
                <option value="aerobics">{ft('classes.typeAerobics')}</option>
                <option value="yoga">{ft('classes.typeYoga')}</option>
                <option value="crossfit">{ft('classes.typeCrossfit')}</option>
              </select>
            </div>
            <div className="grid gap-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-2">
                ...
                <Input type="time" className="nums" value={form.startTime} onChange={(e) => setForm((f) => ({ ...f, startTime: e.target.value }))} />
              </div>
              <div className="grid gap-2">
                <Label>{ft('classes.endTime')}</Label>
                <Input type="time" className="nums" value={form.endTime} onChange={(e) => setForm((f) => ({ ...f, endTime: e.target.value }))} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-2">
        ...
        <DialogContent className="sm:max-w-sm" aria-describedby={undefined}>
          ...
          <div className="grid gap-2 py-2">
            ...
            <MemberSearchCombobox
              selectedMember={enrollMember}
              onSelect={setEnrollMember}
              onClear={() => setEnrollMember(null)}
              disabled={saving}
            />
          </div>
          <DialogFooter>
            ...
            {(rosterClass?.enrollments ?? [])
              ...
              .map((e) => (
                <div key={e.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border p-3 text-sm">
                  <div>
                    <p className="font-medium nums">#{toArabicDigits(e.memberId)}</p>
