    ...
    <div className="relative shrink-0 border-b border-white/10 px-5 py-5">
      ...
      <div className="relative flex flex-col items-center gap-3 text-center">
        ...
        <div>
          ...
          <p className="mt-0.5 truncate text-[13px] font-medium text-white/70">{ui(user?.name ?? t('common.user'))}</p>
        </div>
      </div>
    </div>
  );
...
export function Sidebar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const { t } = useLocale();
  const { user } = useAuth();
  const { canMenuRoute, isReady } = usePermission();
  ...
  const menuRoot = useMemo<MosMenuItem[]>(
    () => {
      if (user?.level !== 2) return MOS_MENU;

      // Employee accounts keep their personal dashboard, but must also see
      // every page that an administrator grants through RBAC exceptions.
      const grantedItems = isReady ? filterMosMenu(MOS_MENU, canMenuRoute) : [];
      return [
        { key: 'myDashboard', icon: 'dashboard', path: workspace?.homeRoute ?? '/profile' },
        ...grantedItems.filter((item) => item.path !== '/dashboard'),
      ];
    },
    [canMenuRoute, isReady, user?.level, workspace?.homeRoute],
  );

  const menu = useMemo(() => {
    const base = user?.level === 2 ? menuRoot : visibleMosMenu(menuRoot, isReady, canMenuRoute);
    return searchMosMenu(base, searchQuery, labelOf);
  }, [canMenuRoute, isReady, searchQuery, labelOf, menuRoot, user?.level]);
