...
const DYNAMIC_PATTERNS: PatternRule[] = [
  ...
  [/^مرحباً، (.+)$/, (x) => `Welcome, ${x}`],
  [/^إعداد جدول (.+)$/, (month) => `Set up schedule for ${month}`],
  [/^(.+) · (.+) · الحالة: (.+)$/, (className, trainerName, status) => `${className} · ${trainerName} · Status: ${status}`],
  [/^(.+) · (.+) · لا توجد مسودة لهذا الشهر$/, (className, trainerName) => `${className} · ${trainerName} · No draft schedule for this month`],
  [/^قائمة انتظار \((\d+)\)$/, (count) => `Waitlist (${count})`],
  [/^(\d+) موعد$/, (count) => `${count} appointments`],
  [/^إضافة موعد يوم (.+)$/, (date) => `Add appointment for ${date}`],
