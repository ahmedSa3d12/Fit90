...
describe('AvailabilityService nutrition appointments', () => {
  ...
  it('calculates starts from provider attendance and service duration', async () => {
    ...
    const result = await service.nutritionAvailableTimes(7, 3, '2099-08-23');
  ...
  it('calculates SPA times from published SPA provider attendance', async () => {
    prisma.club_services.findFirst.mockResolvedValue({
      id: 8, name: 'Massage', category: 'spa', duration_min: 60,
      price: { toString: () => '350.00' }, entitlement_key: null,
    });
    prisma.club_availability_slots.findMany.mockResolvedValue([
      { id: 12, start_time: '15:00', end_time: '17:00' },
    ]);
    prisma.club_bookings.findMany.mockResolvedValue([]);

    const result = await service.providerAvailableTimes(8, 3, '2099-08-23', 'spa');

    expect(result.times).toContainEqual(expect.objectContaining({ availabilitySlotId: 12, windowId: 12, startTime: '15:00', endTime: '16:00' }));
    expect(prisma.club_availability_slots.findMany).toHaveBeenCalledWith(expect.objectContaining({
      where: expect.objectContaining({
        module_type: 'spa',
        monthly_availability: { is: { category: 'spa', status: 'published' } },
      }),
    }));
  });
