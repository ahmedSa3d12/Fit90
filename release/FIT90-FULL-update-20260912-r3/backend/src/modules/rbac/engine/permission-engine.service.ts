...
export class PermissionEngineService {
  ...
  async getEffective(userId: number): Promise<EffectiveResult> {
    ...
    // A general-manager role starts with every permission, but explicit user
    // exceptions may still deny selected capabilities. The legacy level-1
    // system owner above remains an unconditional bypass.
    if (userRoles.some((r) => r.role?.is_super_admin)) {
      const result = await this.superAdminRoleResult(userId, now);
      this.cache.set(userId, result);
      return result;
    }
  ...
  /** General-manager role: full baseline with user-level denials taking priority. */
  private async superAdminRoleResult(userId: number, now: Date): Promise<EffectiveResult> {
    const maps = await this.getCatalogMaps();
    const exceptions = await this.prisma.rbac_user_exceptions.findMany({
      where: { user_id: userId, OR: [{ expires_at: null }, { expires_at: { gt: now } }] },
      select: { resource_id: true, action_id: true, effect: true },
    });
    const exceptionCells: StoredCell[] = exceptions
      .map((exception) => ({
        resourceKey: maps.resourceIdToKey.get(exception.resource_id)!,
        actionKey: maps.actionIdToKey.get(exception.action_id)!,
        effect: exception.effect as 'allow' | 'deny',
      }))
      .filter((exception) => exception.resourceKey && exception.actionKey);

    return {
      superAdmin: false,
      keys: applyExceptions(this.allKeys, exceptionCells),
      scope: new Map(this.flat.map((resource) => [resource.key, 'global'])),
    };
  }
