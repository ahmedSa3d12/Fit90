...
export class ClubSubscriptionsService {
  ...
  async freeze(
    ...
    if (!startDate || !endDate) {
      throw new BadRequestException('يجب تحديد تاريخ بداية ونهاية التجميد');
    }
    if (!/^\d{4}-\d{2}-\d{2}$/.test(startDate) || !/^\d{4}-\d{2}-\d{2}$/.test(endDate)) {
      throw new BadRequestException('صيغة تاريخ التجميد غير صحيحة');
    }
    assertDateOrder(startDate, endDate);
    const today = localDateString();
    if (startDate < today) {
      throw new BadRequestException('لا يمكن بدء التجميد في تاريخ سابق');
    }
    const period = resolveFreezePeriod(startDate, endDate);
    ...
    if (!isOpenEndedEndDate(sub.subscription_end_date) && startDate > sub.subscription_end_date) {
      throw new BadRequestException('يجب أن يبدأ التجميد قبل انتهاء الاشتراك');
    }
    const existingFreeze = await this.prisma.club_subscription_freezes.findFirst({
      where: { subscription_id: id, actual_days: null },
    });
    if (existingFreeze) {
      throw new BadRequestException('يوجد تجميد قائم أو مجدول لهذا الاشتراك بالفعل');
    }
    ...
    const row = await this.prisma.$transaction(async (tx) => {
      await tx.club_subscription_freezes.create({
        data: {
          subscription_id: id,
          freeze_start_date: period.startDate,
          freeze_end_date: period.endDate,
          planned_days: period.days,
          original_end_date: sub.subscription_end_date,
          reason: reason ?? null,
          // A future period is scheduled and becomes active in the nightly job.
          is_active: startsToday,
      ...
      return tx.club_subscriptions.update({
        where: { id },
        data: startsToday ? { status: 'frozen' } : {},
      });
    });
    ...
    await this.audit.log({
      ...
      after: {
        status: startsToday ? 'frozen' : 'scheduled',
        freezeStartDate: period.startDate,
        freezeEndDate: period.endDate,
        plannedDays: period.days,
        reason,
      },
    });
  ...
  async unfreeze(id: number, userId?: number, user?: JwtUser, effectiveEndDate?: string) {
    const sub = await this.prisma.club_subscriptions.findUnique({ where: { id } });
    if (!sub) throw new NotFoundException('الاشتراك غير موجود');
    this.assertBranchAccess(user, sub.branch_id);
    ...
    const today = effectiveEndDate ?? localDateString();
  ...
  /** Starts future-dated freezes on their selected first date. */
  async startScheduledFreezes(today = localDateString()): Promise<number> {
    const scheduled = await this.prisma.club_subscription_freezes.findMany({
      where: {
        is_active: false,
        actual_days: null,
        freeze_start_date: { lte: today },
        freeze_end_date: { gte: today },
      },
      select: { id: true, subscription_id: true },
    });

    for (const freeze of scheduled) {
      await this.prisma.$transaction(async (tx) => {
        await tx.club_subscription_freezes.update({
          where: { id: freeze.id },
          data: { is_active: true },
        ...
        await tx.club_subscriptions.update({
          where: { id: freeze.subscription_id },
          data: { status: 'frozen' },
        });
      });
    }
  ...
  /**
   * Ends freezes only after their selected final date has completed. Passing
   * the scheduled resume date makes a delayed cron run extend by the intended
   * freeze period rather than by the delay.
   */
  async completeDueFreezes(today = localDateString()): Promise<number> {
    const due = await this.prisma.club_subscription_freezes.findMany({
      where: { is_active: true, freeze_end_date: { lt: today } },
      select: { subscription_id: true, freeze_end_date: true },
    });

    for (const freeze of due) {
      if (!freeze.freeze_end_date) continue;
      await this.unfreeze(
        freeze.subscription_id,
        undefined,
        undefined,
        resolveFreezePeriod(freeze.freeze_end_date, freeze.freeze_end_date).unfreezeDate,
      );
    }
    return due.length;
  }
