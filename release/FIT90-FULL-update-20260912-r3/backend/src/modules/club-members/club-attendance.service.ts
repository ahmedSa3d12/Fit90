...
export class ClubAttendanceService {
  ...
  /** Reception entry screen: today's records only, newest first. */
  async listToday(q: ListClubAttendanceDto) {
    const and: Prisma.club_attendanceWhereInput[] = [{ attendance_date: localDateString() }];

    if (q.status && q.status !== 'all') and.push({ status: q.status as 'checked_in' | 'checked_out' });
    if (q.search?.trim()) {
      const search = q.search.trim();
      and.push({
        OR: [{ member_name: { contains: search } }, { member_code: { contains: search } }],
      });
    }

    const where: Prisma.club_attendanceWhereInput = { AND: and };
    ...
    return paginated(rows.map((row) => this.mapRow(row)), total, q.page, q.pageSize);
  }

  /** A permitted employee may check out any open member entry from today. */
  async checkOutToday(attendanceId: number, userId: number) {
    const row = await this.prisma.club_attendance.findFirst({
      where: {
        id: attendanceId,
    ...
    if (!row) throw new NotFoundException('لا يوجد تسجيل دخول لهذا العضو اليوم');
    if (row.status === 'checked_out') throw new BadRequestException('تم تسجيل الخروج مسبقاً');
