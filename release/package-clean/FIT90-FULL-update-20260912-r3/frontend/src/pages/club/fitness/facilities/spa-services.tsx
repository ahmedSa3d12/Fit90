...
export function FitnessSpaServicesPage() {
  ...
  const [form, setForm] = useState({
    ...
    price: '',
    branchId: '',
    serviceKind: 'spa' as 'spa' | 'additional',
  });
  ...
  const openEdit = (row: ClubSpaServiceRow) => {
    ...
    setForm({
      ...
      serviceKind: row.serviceKind ?? 'spa',
    });
    setOpen(true);
  };
  ...
  const columns = useMemo<ColumnDef<ClubSpaServiceRow>[]>(
    () => [
      ...
      {
        accessorKey: 'serviceKind',
        header: ft('spaServices.type'),
        cell: ({ getValue }) =>
          (getValue() as string) === 'additional' ? ft('spaServices.kindAdditional') : ft('spaServices.kindSpa'),
      },
      {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-md" aria-describedby={undefined}>
          ...
          <div className="grid gap-3 py-2">
            ...
            <div className="grid gap-2">
              <Label>{ft('spaServices.type')}</Label>
              <select
                className={SELECT_CLS}
                value={form.serviceKind}
                onChange={(e) => setForm((f) => ({ ...f, serviceKind: e.target.value as 'spa' | 'additional' }))}
              >
                <option value="spa">{ft('spaServices.kindSpa')}</option>
                <option value="additional">{ft('spaServices.kindAdditional')}</option>
              </select>
            </div>
