...
import { canEditLead, editableLeadStatuses } from './lead-conversion';
import { leadDetailValue } from './lead-details';
...
export function MosLeadsPage() {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-wrap gap-1">
          <Button variant="ghost" size="sm" onClick={() => setDetailsLead(row.original)}>
            <Eye className="size-4" /> {isRtl ? 'تفاصيل' : 'Details'}
          </Button>
          {canEditLead(row.original) && (
  ...
  ], [ct, customerSources, isRtl, params.page, params.pageSize, salesReps]);
  ...
  return (
    <div className="space-y-6">
        ...
        toolbar={
          <div className="basis-1/2 flex-1">
            ...
            >
              ...
              {editableLeadStatuses(PIPELINE).map((item) => (
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <div className="grid gap-2">
                ...
                >
                  {editableLeadStatuses(PIPELINE).map((p) => (
      ...
      <Dialog open={detailsLead !== null} onOpenChange={(isOpen) => !isOpen && setDetailsLead(null)}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>{isRtl ? 'تفاصيل العضو المحتمل' : 'Potential Member Details'}</DialogTitle>
          </DialogHeader>
          {detailsLead && (
            <dl className="grid gap-3 py-2 sm:grid-cols-2">
              <LeadDetail label={isRtl ? 'رقم السجل' : 'Record number'} value={String(detailsLead.id)} />
              <LeadDetail label={ct('common.name')} value={detailsLead.name} />
              <LeadDetail label={ct('common.phone')} value={detailsLead.phone} dir="ltr" />
              <LeadDetail label={ct('mos.email')} value={detailsLead.email} dir="ltr" />
              <LeadDetail label={ct('common.status')} value={statusLabel(detailsLead.status)} />
              <LeadDetail
                label={ct('mos.salesEmployee')}
                value={salesReps?.find((employee) => employee.id === detailsLead.assignedTo)?.name}
              />
              <LeadDetail
                label={ct('mos.knowledgeSource')}
                value={customerSources?.find((source) => source.id === detailsLead.sourceId)?.name}
              />
              <LeadDetail label={ct('mos.followUp')} value={detailsLead.followUpAt} />
              <LeadDetail label={ct('common.notes')} value={detailsLead.notes} className="sm:col-span-2" />
            </dl>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setDetailsLead(null)}>{isRtl ? 'إغلاق' : 'Close'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
function LeadDetail({
  ...
  return (
    <div className={`rounded-lg border border-border bg-muted/20 p-3 ${className}`}>
      ...
      <dd className="mt-1 break-words text-sm" dir={dir}>{leadDetailValue(value)}</dd>
    </div>
  );
}
