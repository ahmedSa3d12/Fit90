...
import { rbacActionLabel, rbacNodeName } from '@/lib/rbac-labels';
...
export function PermissionMatrix({ catalog, mode, draft, onChange, baseline, editable }: PermissionMatrixProps) {
  ...
  // Club departments live below one technical `mos` root in the catalog. Show
  // them as first-class selectors in the editor while keeping their original
  // keys and ancestry for permission inheritance.
  const modules = useMemo(
    () => {
      const hasSidebarCatalog = catalog.tree.some((node) => node.key === 'mos' && node.children?.length);
      return catalog.tree.flatMap((node) => {
        if (node.key === 'mos' && node.children?.length) return node.children;
        // These are legacy editor buckets. Their screens are represented by
        // the synchronized Sidebar departments, so showing them duplicates
        // entries and makes the two lists look different.
        if (hasSidebarCatalog && (node.key === 'hr' || node.key === 'app-management')) return [];
        return [node];
      });
    },
    [catalog.tree],
  ...
  const nameMap = useMemo(() => {
    ...
    const walk = (nodes: RbacNode[]) =>
      nodes.forEach((n) => {
        m.set(n.key, rbacNodeName(n, locale));
  ...
  useEffect(() => {
    // Show every screen below the selected administration immediately, while
    // preserving the user's expand/collapse choices in other administrations.
    setCollapsed((previous) => {
      const selectedModule = modules.find((module) => module.key === activeKey) ?? modules[0];
      if (!selectedModule) return previous;
      const next = expandSelectedModule(previous, selectedModule);
  ...
  const toggleCell = (resourceKey: string, actionKey: string) => {
    ...
    setCell(resourceKey, actionKey, cycle(current));
  };
  ...
  /**
   * Row "All": if every applicable cell in scope is explicit allow → clear to inherit; else set all allow.
  ...
  const toggleRowAll = (node: RbacNode) => {
    ...
    const allAllowed = targets.every((n) => n.actions.every((a) => map.get(cellId(n.key, a)) === 'allow'));
    for (const n of targets) {
      for (const a of n.actions) {
        const k = cellId(n.key, a);
        if (allAllowed) map.delete(k);
        else map.set(k, 'allow');
      }
    }
    onChange(map);
  };
    ...
    <TooltipProvider delayDuration={200}>
      <div className="flex min-h-[70vh] flex-col gap-0 md:flex-row">
        ...
        <aside className="hidden w-60 shrink-0 border-e border-border bg-muted/20 p-2 md:block">
          <p className="px-2 py-2 text-xs text-muted-foreground">{ui('اختاري الإدارة أو الوحدة لتحديد صلاحياتها.')}</p>
            ...
            {modules.map((m) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="flex-1 truncate text-start">{rbacNodeName(m, locale)}</span>
                  {moduleHasGrant(m) && (
          ...
          <table className="w-full border-collapse text-sm">
            <thead className="sticky top-0 z-10 bg-card shadow-sm">
              <tr className="border-b border-border">
                <th className="sticky start-0 z-20 bg-card px-3 py-2 text-start font-semibold">{ui('الإدارة / الشاشة')}</th>
                ...
                {columns.map((a) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="mt-0.5 block text-[11px]">{def ? rbacActionLabel(def, locale) : a}</span>
                    </th>
                  );
                })}
              </tr>
                  ...
                  <tr key={node.key} className={cn('border-b border-border/60 hover:bg-accent/40', DEPTH_BG[Math.min(depth, 2)])}>
                    <td className="sticky start-0 z-[1] px-3 py-1.5" style={{ paddingInlineStart: 12 + depth * 18 }}>
                      <div className={cn('flex items-center gap-2', DEPTH_BG[Math.min(depth, 2)])}>
                        ...
                        <span className={cn('truncate', depth === 0 && 'font-semibold')}>{rbacNodeName(node, locale)}</span>
                        {node.type === 'page' && (
