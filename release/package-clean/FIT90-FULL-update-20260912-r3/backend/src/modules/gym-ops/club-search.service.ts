...
export class ClubSearchService {
  ...
  async universalSearch(q: string, branchId?: number, limit = 10): Promise<ClubSearchHit[]> {
    const query = q.trim();
    ...
    const hits: ClubSearchHit[] = members.map((m) => {
      ...
      return {
        ...
        subscriptionStatus: active
          ? deriveSubStatus(active.subscription_start_date, active.subscription_end_date)
          : memberSubs[0]
            ? deriveSubStatus(memberSubs[0].subscription_start_date, memberSubs[0].subscription_end_date)
            : null,
  ...
  async recentCheckIns(limit = 20, branchId?: number) {
  ...
  async searchWithEntitlement(q: string, branchId?: number) {
    const hits = await this.universalSearch(q, branchId, 10);
    const top = hits[0];
