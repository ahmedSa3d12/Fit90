...
export class MeController {
  constructor(
    ...
    private readonly staffDashboardsService: StaffDashboardsService,
  ...
  workspaceWidgets(@CurrentUser('sub') userId: number) {
    return this.workspaceWidgetsService.getWidgets(userId);
  }

  /** Private dashboard for the trainer linked to the current login account. */
  @Get('trainer-dashboard')
  trainerDashboard(
    @CurrentUser('sub') userId: number,
    @Query('month') month?: string,
  ) {
    return this.trainerDashboardService.getDashboard(userId, month);
  }

  @Get('sales-dashboard')
  salesDashboard(@CurrentUser('sub') userId: number, @Query('month') month?: string) {
    return this.staffDashboardsService.salesDashboard(userId, month);
  }

  @Get('nutrition-dashboard')
  nutritionDashboard(@CurrentUser('sub') userId: number, @Query('month') month?: string) {
    return this.staffDashboardsService.nutritionDashboard(userId, month);
  }

  @Get('spa-dashboard')
  spaDashboard(@CurrentUser('sub') userId: number, @Query('month') month?: string) {
    return this.staffDashboardsService.spaDashboard(userId, month);
  }
