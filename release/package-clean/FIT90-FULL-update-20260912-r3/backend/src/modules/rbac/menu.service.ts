...
export class MenuService {
  ...
  */
  async getMenu(userId: number): Promise<MenuNode[]> {
    ...
    if (!isSystemAdmin && perms.length === 0) return [];

    const grantedIds = perms.map((p) => p.page_id_fk);

    const pages = await this.prisma.pages.findMany({
      ...(isSystemAdmin ? {} : { where: { page_id: { in: grantedIds } } }),
      orderBy: { page_order: 'asc' },
    });
    const rootIds = isSystemAdmin
      ? new Set(pages.filter((page) => page.group_id_fk === 0).map((page) => page.page_id))
      : new Set(perms.filter((p) => p.page_level === 0).map((p) => p.page_id_fk));
