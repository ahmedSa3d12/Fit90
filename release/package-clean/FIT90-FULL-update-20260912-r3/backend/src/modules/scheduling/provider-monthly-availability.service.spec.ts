...
describe('ProviderMonthlyAvailabilityService', () => {
  ...
  it('creates a draft spa monthly plan for its provider without choosing a service', async () => {
    prisma.club_trainers.findFirst.mockResolvedValue({
      id: 22, employee_id: 220, name: 'Salma', is_active: true, specialization: 'Spa therapist',
    });
    prisma.employees.findFirst.mockResolvedValue({ id: 220, branch_id_fk: 1 });
    prisma.club_provider_monthly_availabilities.create.mockResolvedValue({
      id: 4, employee_id: 22, category: 'spa', month: 9, year: 2026,
      status: 'draft', published_at: null, windows: [],
    });

    const result = await service.create({ employeeId: 22, month: 9, year: 2026, category: 'spa' } as any);

    expect(result).toEqual(expect.objectContaining({ id: 4, employeeId: 22, status: 'draft', category: 'spa' }));
    expect(prisma.club_provider_monthly_availabilities.create).toHaveBeenCalledWith(expect.objectContaining({
      data: { employee_id: 22, category: 'spa', month: 9, year: 2026 },
    }));
  });
  it('creates personal-training attendance without a service and preserves its capacity', async () => {
    prisma.club_trainers.findFirst.mockResolvedValue({
      id: 25, employee_id: 250, name: 'Coach Ali', is_active: true, specialization: 'Personal training',
    });
    prisma.employees.findFirst.mockResolvedValue({ id: 250, branch_id_fk: 1 });
    prisma.club_provider_monthly_availabilities.create.mockResolvedValue({
      id: 5, employee_id: 25, category: 'personal_training', month: 9, year: 2026,
      status: 'draft', published_at: null, windows: [],
    });
    prisma.club_provider_monthly_availabilities.findUnique.mockResolvedValueOnce(null).mockResolvedValue({
      id: 5, employee_id: 25, category: 'personal_training', month: 9, year: 2026,
      status: 'draft', windows: [],
    });
    prisma.club_availability_slots.findFirst.mockResolvedValue(null);

    await service.create({ employeeId: 25, month: 9, year: 2026, category: 'personal_training' } as any);
    await service.addWindows(5, {
      slotDate: '2026-09-20', startTime: '18:00', endTime: '19:00', capacity: 4,
    } as any);

    expect(prisma.club_provider_monthly_availabilities.create).toHaveBeenCalledWith(expect.objectContaining({
      data: { employee_id: 25, category: 'personal_training', month: 9, year: 2026 },
    }));
    expect(prisma.club_availability_slots.create).toHaveBeenCalledWith({ data: expect.objectContaining({
      module_type: 'personal_training', service_id: null, capacity: 4,
    }) });
  });
  ...
  it('cancels a published window with an audit reason', async () => {
    ...
    prisma.club_availability_slots.findFirst.mockResolvedValue({
      ...
      slot_date: '2099-08-23', start_time: '15:00', end_time: '20:00',
    });
    ...
    prisma.club_bookings.findMany.mockResolvedValue([{
      ...
      schedule: { id: 41, slot_date: '2099-08-23', start_time: '15:00' },
      service: { name: 'InBody' },
    }]);
