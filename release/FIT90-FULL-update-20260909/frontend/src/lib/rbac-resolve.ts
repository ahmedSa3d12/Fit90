...
/**
 * Opens every nested group in the administration the user has just selected.
 * Collapsed nodes from other administrations are intentionally preserved.
 */
export function expandSelectedModule(collapsed: ReadonlySet<string>, module: RbacNode): Set<string> {
...
/** Next state in the cycle inherit -> allow -> deny -> inherit. */
export function cycle(state: TriState): TriState {
  return state === 'inherit' ? 'allow' : state === 'allow' ? 'deny' : 'inherit';
}
