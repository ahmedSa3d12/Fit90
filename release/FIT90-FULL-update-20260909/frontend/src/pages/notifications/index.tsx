    ...
    },
    {
      ...
      cell: ({ row }) => (
        <button
          ...
          onClick={() => void markRead(row.original)}
        >
          {row.original.title ?? '—'}
        </button>
      ),
    ...
    { accessorKey: 'body', header: ui('التفاصيل'), cell: ({ getValue }) => <span className="text-muted-foreground">{getValue() as string || '—'}</span> },
    {
      ...
      cell: ({ row }) => (
        <span className="nums text-muted-foreground">
          ...
          {row.original.time ? ` ${toArabicDigits(row.original.time)}` : ''}
        </span>
      ),
    },
    {
  ...
  return (
    <div>
      <PageHeader
        ...
        description={ui('انتهاء العقود والإقامة والتأمين · أعياد الميلاد · التجربة')}
        actions={
