import { Bell, CalendarDays, LogOut, Menu, Search, User } from 'lucide-react';
...
export function Topbar({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 overflow-hidden border-b border-border/60 bg-card/90 shadow-sm backdrop-blur-xl dark:border-white/[0.06] dark:bg-[hsl(222_17%_10%_/0.88)]">
      ...
      <div className="relative flex h-[4.75rem] items-center gap-3 px-4 md:px-6">
        <Button variant="ghost" size="icon" className="shrink-0 lg:hidden" onClick={onMenuClick} aria-label={t('common.menu')}>
          <Menu />
        </Button>

        <div className="hidden min-w-0 max-w-[220px] flex-col justify-center lg:flex">
        ...
        <div className="relative hidden min-w-0 flex-1 md:block md:max-w-md lg:max-w-xl">
          ...
          <input
            className="relative h-11 w-full rounded-xl border border-primary/15 bg-background/80 pe-10 ps-4 text-sm text-foreground shadow-sm outline-none transition-all placeholder:text-muted-foreground focus:border-primary/40 focus:bg-background focus:ring-2 focus:ring-primary/15 dark:text-white dark:placeholder:text-white/55"
