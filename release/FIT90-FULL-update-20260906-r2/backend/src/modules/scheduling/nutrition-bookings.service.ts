  ...
  async createPersonalTrainingAttendance(dto: CreatePersonalTrainingBookingDto, user?: JwtUser) {
    return this.prisma.$transaction(async (tx: any) => {
      ...
      return {
        ...
        // Personal-training booking has no member-selected service. The internal
        // service record is only used to consume the PT entitlement, so never
        // expose a configured service label (for example, "زومبا") to the member.
        serviceName: 'تدريب شخصي',
