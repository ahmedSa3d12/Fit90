  ...
  private async runAction(
    ...
    switch (action.type) {
      ...
      case 'send_notification': {
        if (payload.memberId) {
          const title = this.interpolate(action.notificationTitle ?? 'تنبيه من النادي', payload);
          const message = this.interpolate(action.notificationBody ?? title, payload);
          await this.prisma.club_notifications.create({
            data: {
              // The schema migration adds this enum value; the cast keeps hot
              // development servers with an older generated Prisma client compatible.
              type: 'member_message' as never,
