...
import { remainingAmount, toClubPaymentMethod, toNum, nextSeqFromMax } from './club-subscription.utils';
    ...
    const receipt = await retryOnUniqueViolation(async () => {
      ...
      return client.club_receipts.create({
        data: {
          ...
          payment_method: toClubPaymentMethod(opts.paymentMethod) ?? sub.payment_method ?? null,
