...
type LookupRow = { id: number; name: string; nameEn?: string | null };
...
export function ClubPackageSettingsPage() {
  ...
  const { data: packageTypes } = useArrayResource<LookupRow>('club-mos/lookups/package_type');

  const orderedPackageTypes = useMemo(() => {
    const types = [...(packageTypes ?? [])];
    return types.sort((a, b) => {
      const aIsGym = [a.name, a.nameEn].some((value) => value?.trim().toLowerCase() === 'gym');
      const bIsGym = [b.name, b.nameEn].some((value) => value?.trim().toLowerCase() === 'gym');
      return Number(bIsGym) - Number(aIsGym);
    });
  }, [packageTypes]);

  const gymPackageTypeId = useMemo(
    () => orderedPackageTypes.find((type) =>
      [type.name, type.nameEn].some((value) => value?.trim().toLowerCase() === 'gym'),
    )?.id,
    [orderedPackageTypes],
  );
  ...
  useEffect(() => {
    if (!subTypeOpen || subTypeEditId != null || subTypeForm.packageType || gymPackageTypeId == null) return;
    setSubTypeForm((form) => ({ ...form, packageType: String(gymPackageTypeId) }));
  }, [subTypeOpen, subTypeEditId, subTypeForm.packageType, gymPackageTypeId]);
  ...
  const saveSubType = async () => {
    if (!subTypeForm.nameAr.trim() || !subTypeForm.nameEn.trim() || !subTypeForm.packageType || !subTypeForm.durationValue || subTypeForm.price === '') {
    ...
    try {
      ...
      const body = {
        ...
        packageType: subTypeForm.packageType || null,
          ...
          <FormDialogBody>
            <DialogFormSection title={ct('packages.sectionBasic')} icon={Package}>
              <DialogFormGrid columns={2}>
                ...
                <FormField label={ct('packages.packageType')} required>
                  <Select value={subTypeForm.packageType} onValueChange={(packageType) => patchSubTypeForm({ packageType })}>
                    <SelectTrigger><SelectValue placeholder={ct('packages.selectPackageType')} /></SelectTrigger>
                    <SelectContent>
                      {orderedPackageTypes.map((type) => <SelectItem key={type.id} value={String(type.id)}>{type.name}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </FormField>
                <div />
