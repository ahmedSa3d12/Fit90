...
import { workspaceWidgetTitle } from './workspace-widget-labels';
...
function TreasuryWidget({ data }: { data: unknown }) {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground dark:text-white/75">{toArabicDigits(d?.receiptCount ?? 0)} {uiStatic('إيصال اليوم')}</p>
    </div>
  );
}
