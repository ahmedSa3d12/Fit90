...
type ProviderTimesResponse = {
  times: Array<{ windowId: number; startTime: string; endTime: string }>;
};

type ProviderWindowEditor = {
  planId: string;
  slotDate: string;
  startTime: string;
  endTime: string;
...
function SlotBookingDialog({ slot, onClose, onDone }: { slot: ScheduleRow | null; onClose: () => void; onDone: () => void }) {
  ...
  const [spaServiceId, setSpaServiceId] = useState('');
  const [spaSelectedTime, setSpaSelectedTime] = useState<{ windowId: number; startTime: string; endTime: string } | null>(null);
  const { items: allServices } = useFitnessResourceList<ServiceRow>('scheduling/services');
  const spaBooking = slot?.category === 'spa';
  const personalTrainingBooking = slot?.category === 'personal_training';
  const providerServices = allServices.filter((service) => service.category === 'spa' && service.isActive);
  ...
  const { data: spaEligibility, isFetching: spaEligibilityLoading } = useQuery({
    queryKey: ['spa-booking-eligibility', slot?.id, member?.id, spaServiceId],
    enabled: spaBooking && member != null && Boolean(spaServiceId),
    queryFn: async () => (await api.get<{ coverageType: 'subscription' | 'pay_at_branch'; amountDue: string; total: number; used: number; reserved: number; remaining: number }>('/scheduling/bookings/spa/eligibility', {
      params: { memberId: member!.id, serviceId: spaServiceId, date: slot!.slotDate },
    })).data,
  });
  const { data: personalTrainingEligibility, isFetching: personalTrainingEligibilityLoading } = useQuery({
    queryKey: ['personal-training-booking-eligibility', slot?.id, member?.id],
    enabled: personalTrainingBooking && member != null,
    queryFn: async () => (await api.get<{ coverageType: 'subscription' | 'pay_at_branch'; total: number; used: number; reserved: number; remaining: number; hasPersonalTrainingSubscription: boolean }>('/scheduling/bookings/personal-training/eligibility', {
      params: { memberId: member!.id, date: slot!.slotDate },
    })).data,
  });
  const { data: spaTimes, isFetching: spaTimesLoading } = useQuery({
    queryKey: ['spa-booking-times', slot?.id, slot?.employeeId, slot?.slotDate, spaServiceId],
    enabled: spaBooking && Boolean(spaServiceId),
    queryFn: async () => (await api.get<ProviderTimesResponse>('/availability/provider-times', {
      params: { serviceId: spaServiceId, trainerId: slot!.employeeId, date: slot!.slotDate, category: 'spa' },
    })).data,
  });
  const spaAvailableTimes = (spaTimes?.times ?? []).filter((time) => time.windowId === slot?.id);
  useEffect(() => {
  ...
  useEffect(() => {
    setSpaServiceId('');
    setSpaSelectedTime(null);
  }, [slot?.id]);
  useEffect(() => setSpaSelectedTime(null), [spaServiceId]);
  const save = async () => {
    if (!slot || !member || (spaBooking && (!spaServiceId || !spaSelectedTime))) return;
    setSaving(true);
    try {
      const status = slot.remaining > 0 ? 'confirmed' : 'wait';
      const response = spaBooking
        ? await api.post<{ coverageType: string; remaining: number }>('/scheduling/bookings/spa', { availabilitySlotId: spaSelectedTime!.windowId, serviceId: Number(spaServiceId), memberId: member.id, startTime: spaSelectedTime!.startTime, status: 'confirmed' })
        : personalTrainingBooking
          ? await api.post<{ coverageType: string; remaining: number }>('/scheduling/bookings/personal-training', { availabilitySlotId: slot.id, memberId: member.id, status: 'confirmed' })
        : await api.post('/scheduling/bookings', { scheduleId: slot.id, memberId: member.id, memberName: member.name, status, additionalServiceIds: selectedAdditionalIds });
      toast.success(ui((spaBooking || personalTrainingBooking) && response.data.coverageType === 'subscription' ? `تم تأكيد الحجز من الرصيد، والمتبقي ${response.data.remaining}` : status === 'confirmed' ? 'تم تأكيد الحجز' : 'تمت إضافة العضو إلى قائمة الانتظار'));
      onDone();
  ...
  return <Dialog open={slot != null} onOpenChange={(open) => !open && onClose()}><DialogContent size="md" aria-describedby={undefined}><DialogHeader><DialogTitle>{ui('بيانات الحجز')}</DialogTitle></DialogHeader>{slot && <div className="space-y-4">
    <div className="rounded-xl border bg-muted/20 p-4"><b>{personalTrainingBooking ? ui('فترة حضور تدريب شخصي') : slot.serviceName}</b><p>{slot.employeeName} · {slotDateTime(slot).toLocaleString(dateLocale())}</p><p className="mt-1 text-sm">{ui('الحالة المتوقعة')}: <b>{statusText(slot.remaining > 0 ? 'confirmed' : 'wait', ui)}</b></p></div>
    ...
    {spaBooking && <div><Label>{ui('خدمة السبا')}</Label><select className={SELECT_CLS} value={spaServiceId} onChange={(event) => setSpaServiceId(event.target.value)}><option value="">{ui('اختر الخدمة')}</option>{providerServices.map((service) => <option key={service.id} value={service.id}>{service.name} — {Number(service.price).toFixed(2)}</option>)}</select></div>}
    {spaBooking && spaServiceId && <div><Label>{ui('وقت الموعد')}</Label>{spaTimesLoading ? <p className="mt-2 text-sm text-muted-foreground">{ui('جارٍ تحميل المواعيد المتاحة...')}</p> : spaAvailableTimes.length === 0 ? <p className="mt-2 rounded-lg border border-dashed p-3 text-sm text-muted-foreground">{ui('لا توجد مواعيد متاحة لهذه الخدمة داخل فترة الحضور')}</p> : <div className="mt-2 flex flex-wrap gap-2">{spaAvailableTimes.map((time) => <Button key={`${time.windowId}-${time.startTime}`} type="button" variant={spaSelectedTime?.startTime === time.startTime ? 'default' : 'outline'} onClick={() => setSpaSelectedTime(time)}><Clock3 className="me-1.5 size-4" />{time.startTime.slice(0, 5)} – {time.endTime.slice(0, 5)}</Button>)}</div>}</div>}
    {spaBooking && member && spaServiceId && <div className="rounded-xl border border-primary/30 bg-primary/5 p-4">{spaEligibilityLoading ? <p className="text-sm text-muted-foreground">{ui('جارٍ فحص رصيد جلسات السبا المجانية...')}</p> : spaEligibility?.coverageType === 'subscription' ? <p className="font-semibold text-emerald-700 dark:text-emerald-300">{ui('سيتم الخصم من الرصيد المجاني')} · {ui('الإجمالي')}: {spaEligibility?.total ?? 0} · {ui('المستخدم')}: {spaEligibility?.used ?? 0} · {ui('المحجوز')}: {spaEligibility?.reserved ?? 0} · {ui('المتبقي')}: {spaEligibility?.remaining ?? 0}</p> : <p className="font-semibold text-amber-700 dark:text-amber-300">{ui('لا يوجد رصيد مجاني؛ الحجز سيكون مدفوعًا')}</p>}</div>}
    {personalTrainingBooking && member && <div className="rounded-xl border border-primary/30 bg-primary/5 p-4">{personalTrainingEligibilityLoading ? <p className="text-sm text-muted-foreground">{ui('جارٍ فحص اشتراك التدريب الشخصي...')}</p> : !personalTrainingEligibility?.hasPersonalTrainingSubscription ? <p className="font-semibold text-amber-700 dark:text-amber-300">{ui('لا توجد اشتراكات تدريب شخصي متاحة لهذا العضو')}</p> : personalTrainingEligibility.coverageType === 'subscription' ? <p className="font-semibold text-emerald-700 dark:text-emerald-300">{ui('اشتراك التدريب الشخصي')} · {ui('الإجمالي')}: {personalTrainingEligibility.total} · {ui('المستخدم')}: {personalTrainingEligibility.used} · {ui('المتبقي')}: {personalTrainingEligibility.remaining}</p> : <p className="font-semibold text-amber-700 dark:text-amber-300">{ui('لا يوجد رصيد متبقٍ في اشتراك التدريب الشخصي')}</p>}</div>}
    ...
    <div className="flex justify-end gap-2"><Button variant="outline" onClick={onClose}>{ui('إلغاء')}</Button><Button disabled={!member || saving || (spaBooking && (!spaServiceId || !spaSelectedTime || spaEligibilityLoading || spaTimesLoading)) || (personalTrainingBooking && (slot.remaining <= 0 || personalTrainingEligibilityLoading || personalTrainingEligibility?.coverageType !== 'subscription'))} onClick={() => void save()}>{ui(personalTrainingBooking || slot.remaining > 0 ? 'تأكيد الحجز' : 'إضافة إلى الانتظار')}</Button></div>
...
export function DepartmentAppointmentsAdmin({ category }: { category: Category }) {
  ...
  const [spaWindowEditor, setSpaWindowEditor] = useState<ProviderWindowEditor | null>(null);
  const [savingSpaWindow, setSavingSpaWindow] = useState(false);
  ...
  const providerOnlySchedule = category === 'spa' || category === 'personal_training';
  const providerOnlyFilters = providerOnlySchedule;
  ...
  const { data: spaPlans = [], isFetching: spaPlansLoading, refetch: refetchSpaPlans } = useQuery({
    queryKey: ['provider-booking-board', category, applied.employeeId, applied.month, applied.year],
    enabled: providerOnlySchedule,
    queryFn: async () => (await api.get<Array<{ id: number; employeeId: number; month: number; year: number; trainer: { name: string }; windows?: Array<{ id: number; slot_date: string; start_time: string; end_time: string; booking_start_at?: string | null; booking_end_at?: string | null; is_active: boolean; cancelled_at?: string | null; bookingCount?: number; capacity?: number }> }>>('/scheduling/provider-monthly-availability', { params: { category, employeeId: applied.employeeId || undefined, month: applied.month, year: applied.year, status: 'published' } })).data,
  });
  const displayedProviderSlots: ScheduleRow[] = providerOnlySchedule ? spaPlans.flatMap((plan) => (plan.windows ?? []).map((window) => ({
    id: window.id, monthlyScheduleId: plan.id, serviceId: 0, serviceName: category === 'spa' ? ui('فترة حضور سبا') : ui('فترة حضور تدريب شخصي'), category, color: null,
    employeeId: plan.employeeId, employeeName: plan.trainer.name, roomId: null, roomName: null, machineId: null, branchId: null,
    slotDate: window.slot_date, startTime: window.start_time, endTime: window.end_time,
    bookingStartAt: window.booking_start_at ?? null, bookingEndAt: window.booking_end_at ?? null,
    // SPA windows contain several appointment times; personal-training windows use their configured capacity.
    capacity: category === 'personal_training' ? window.capacity ?? 1 : 1,
    bookedCount: window.bookingCount ?? 0,
    remaining: window.cancelled_at || !window.is_active ? 0 : category === 'personal_training' ? Math.max(0, (window.capacity ?? 1) - (window.bookingCount ?? 0)) : 1,
    status: window.cancelled_at || !window.is_active ? 'cancelled' : 'available', notes: 'provider-availability',
  }))) : slots;
  ...
  const { data: detailResponse, isFetching: detailsLoading } = useQuery({
    ...
    queryFn: async () => (await api.get<{ data: BookingDetails[] }>('/scheduling/bookings', { params: providerOnlySchedule ? { availabilitySlotId: detailsSlot!.id, page: 1, pageSize: 200 } : { scheduleId: detailsSlot!.id, page: 1, pageSize: 200 } })).data,
  });
  ...
  const saveEdit = async () => {
    if (!editSlot) return;
    try {
      const payload = { slotDate: editForm.slotDate, startTime: editForm.startTime, endTime: editForm.endTime, bookingStartAt: editForm.bookingStartAt ? new Date(editForm.bookingStartAt).toISOString() : undefined, bookingEndAt: editForm.bookingEndAt ? new Date(editForm.bookingEndAt).toISOString() : undefined, ...(category === 'personal_training' ? { capacity: Number(editForm.capacity) } : {}) };
      if (providerOnlySchedule) await api.patch(`/scheduling/provider-monthly-availability/${editSlot.monthlyScheduleId}/windows/${editSlot.id}`, payload);
      else await api.put(`/scheduling/schedules/${editSlot.id}`, { ...payload, capacity: Number(editForm.capacity) });
      toast.success(ui('تم تعديل الموعد'));
      setEditSlot(null);
      setSelectedSlot(null);
      await refetch();
      if (providerOnlySchedule) await refetchSpaPlans();
      if (boardDraftPlanId) await refetchBoardDraft();
    } catch (error) { toast.error(apiError(error)); }
  };
  const openSpaAdd = () => {
    if (spaPlans.length === 0) {
      toast.error(ui(category === 'spa' ? 'لا يوجد جدول سبا منشور لإضافة موعد إليه' : 'لا يوجد جدول تدريب شخصي منشور لإضافة موعد إليه'));
      return;
    }
    setSpaWindowEditor({
      planId: String(spaPlans[0]!.id),
      slotDate: `${applied.year}-${String(applied.month).padStart(2, '0')}-01`,
      startTime: '15:00', endTime: '16:00', bookingStartAt: '', bookingEndAt: '', capacity: '1',
    });
  };
  const saveSpaAdd = async () => {
    if (!spaWindowEditor) return;
    if (!spaWindowEditor.slotDate || !spaWindowEditor.startTime || !spaWindowEditor.endTime || spaWindowEditor.endTime <= spaWindowEditor.startTime) {
      toast.error(ui('تأكد أن نهاية فترة الحضور بعد بدايتها'));
      return;
    }
    if (category === 'personal_training' && Number(spaWindowEditor.capacity) < 1) {
      toast.error(ui('السعة يجب أن تكون 1 على الأقل'));
      return;
    }
    if ((spaWindowEditor.bookingStartAt === '') !== (spaWindowEditor.bookingEndAt === '')
      || (spaWindowEditor.bookingStartAt && spaWindowEditor.bookingEndAt && spaWindowEditor.bookingEndAt <= spaWindowEditor.bookingStartAt)) {
      toast.error(ui('حدد فتح وغلق الحجز معًا، ويجب أن يكون الغلق بعد الفتح'));
      return;
    }
    setSavingSpaWindow(true);
    try {
      await api.post(`/scheduling/provider-monthly-availability/${spaWindowEditor.planId}/windows`, {
        slotDate: spaWindowEditor.slotDate,
        startTime: spaWindowEditor.startTime,
        endTime: spaWindowEditor.endTime,
        repeatWeekly: false,
        bookingStartAt: spaWindowEditor.bookingStartAt ? new Date(spaWindowEditor.bookingStartAt).toISOString() : undefined,
        bookingEndAt: spaWindowEditor.bookingEndAt ? new Date(spaWindowEditor.bookingEndAt).toISOString() : undefined,
        ...(category === 'personal_training' ? { capacity: Number(spaWindowEditor.capacity) } : {}),
      });
      toast.success(ui(category === 'spa' ? 'تمت إضافة فترة الحضور إلى جدول السبا' : 'تمت إضافة فترة الحضور إلى جدول التدريب الشخصي'));
      setSpaWindowEditor(null);
      await refetchSpaPlans();
    } catch (error) { toast.error(apiError(error)); } finally { setSavingSpaWindow(false); }
  };
  ...
  const cancelSlot = async (slot: ScheduleRow) => {
    if (providerOnlySchedule) {
      const reason = globalThis.prompt(ui('سبب إلغاء فترة الحضور'));
      if (!reason?.trim()) return;
      try {
        await api.post(`/scheduling/provider-monthly-availability/${slot.monthlyScheduleId}/windows/${slot.id}/cancel`, { reason: reason.trim() });
        toast.success(ui('تم إلغاء فترة الحضور والحجوزات المرتبطة'));
        setDetailsSlot(null);
        setSelectedSlot(null);
        await refetchSpaPlans();
      } catch (error) { toast.error(apiError(error)); }
  ...
  const cancelBooking = async (booking: BookingDetails) => {
    ...
    try {
      await api.patch(category === 'spa' ? `/scheduling/bookings/spa/${booking.id}/status` : category === 'personal_training' ? `/scheduling/bookings/personal-training/${booking.id}/status` : `/scheduling/bookings/${booking.id}`, { status: 'cancelled' });
  ...
  const completeBooking = async (booking: BookingDetails) => {
    try {
      await api.patch(category === 'spa' ? `/scheduling/bookings/spa/${booking.id}/status` : category === 'personal_training' ? `/scheduling/bookings/personal-training/${booking.id}/status` : `/scheduling/bookings/${booking.id}`, { status: 'completed' });
  ...
  const publishedSlots = displayedProviderSlots.filter((slot) => slot.status !== 'hidden');
  ...
  return <div className="space-y-5">
    <Filters category={category} providerOnly={providerOnlyFilters} value={filters} onChange={setFilters} onApply={() => { setDraftPlanId(null); setApplied({ ...filters, serviceId: providerOnlyFilters ? '' : filters.serviceId }); if (JSON.stringify(applied) === JSON.stringify(filters)) void refetch(); }} />

    {!providerOnlyFilters && <Card className="border-primary/30 bg-gradient-to-l from-primary/10 via-card to-card">
    ...
    <Card className="overflow-hidden">
      <div className="border-b bg-muted/20 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {providerOnlySchedule && <Button onClick={openSpaAdd}><Plus className="me-1.5 size-4" />{ui('إضافة موعد')}</Button>}
      ...
      {(isFetching || spaPlansLoading || boardDraftLoading) && <div className="border-b bg-primary/5 p-3 text-center text-sm text-primary">{ui('جارٍ تحميل مواعيد الشهر...')}</div>}
          ...
          {Array.from({ length: daysCount }, (_, index) => index + 1).map((day) => {
            ...
            return <div key={day} className="min-h-44 border-b border-e bg-card p-2 align-top">
              ...
              <div className="space-y-2">{daySlots.map((slot) => <button key={slot.id} type="button" onClick={() => showingDraft ? openEdit(slot) : setSelectedSlot(slot)} className={`w-full rounded-lg border p-2 text-start transition hover:-translate-y-0.5 hover:shadow-md ${showingDraft ? 'border-amber-500/45 bg-amber-500/10 text-amber-700 dark:text-amber-300' : slotTone(slot)}`}>
                ...
                <span className="mt-1 block text-[10px]">{showingDraft ? `${ui('مسودة')} · ${ui('السعة')} ${slot.capacity} · ${ui('اضغط للتعديل')}` : category === 'spa' ? `${visualSlotLabel(slot)} · ${slot.bookedCount} ${ui('حجز')}` : `${visualSlotLabel(slot)} · ${ui('متبقي')} ${slot.remaining}`}</span>
              </button>)}</div>
            </div>;
          })}
        </div>
      </div>
      {!isFetching && !spaPlansLoading && !boardDraftLoading && displayedSlots.length === 0 && <div className="border-t p-8 text-center text-muted-foreground">{ui(showingDraft ? 'مسودة هذا الشهر لا تحتوي على مواعيد حتى الآن.' : 'لا توجد مواعيد منشورة مطابقة للاختيارات في هذا الشهر.')}</div>}
    </Card>
    ...
    <SlotBookingDialog slot={bookingSlot} onClose={() => setBookingSlot(null)} onDone={() => { setBookingSlot(null); void refetch(); void refetchSpaPlans(); }} />
    <Dialog open={spaWindowEditor != null} onOpenChange={(open) => !open && setSpaWindowEditor(null)}><DialogContent size="md" aria-describedby={undefined}><DialogHeader><DialogTitle>{ui(category === 'spa' ? 'إضافة موعد إلى جدول السبا المنشور' : 'إضافة موعد إلى جدول التدريب الشخصي المنشور')}</DialogTitle></DialogHeader>{spaWindowEditor && <div className="space-y-4"><div><Label>{ui(category === 'spa' ? 'جدول مقدم خدمة السبا' : 'جدول مدرب التدريب الشخصي')}</Label><select className={SELECT_CLS} value={spaWindowEditor.planId} onChange={(event) => setSpaWindowEditor({ ...spaWindowEditor, planId: event.target.value })}>{spaPlans.map((plan) => <option key={plan.id} value={plan.id}>{plan.trainer.name} · {plan.month}/{plan.year}</option>)}</select></div><div className={`grid gap-3 ${category === 'personal_training' ? 'sm:grid-cols-4' : 'sm:grid-cols-3'}`}><div><Label>{ui('التاريخ')}</Label><Input type="date" value={spaWindowEditor.slotDate} onChange={(event) => setSpaWindowEditor({ ...spaWindowEditor, slotDate: event.target.value })} /></div><div><Label>{ui('من الساعة')}</Label><Input type="time" value={spaWindowEditor.startTime} onChange={(event) => setSpaWindowEditor({ ...spaWindowEditor, startTime: event.target.value })} /></div><div><Label>{ui('إلى الساعة')}</Label><Input type="time" value={spaWindowEditor.endTime} onChange={(event) => setSpaWindowEditor({ ...spaWindowEditor, endTime: event.target.value })} /></div>{category === 'personal_training' && <div><Label>{ui('السعة')}</Label><Input type="number" min="1" value={spaWindowEditor.capacity} onChange={(event) => setSpaWindowEditor({ ...spaWindowEditor, capacity: event.target.value })} /></div>}</div><div className="grid gap-3 sm:grid-cols-2"><div><Label>{ui('فتح الحجز (اختياري)')}</Label><Input type="datetime-local" value={spaWindowEditor.bookingStartAt} onChange={(event) => setSpaWindowEditor({ ...spaWindowEditor, bookingStartAt: event.target.value })} /></div><div><Label>{ui('غلق الحجز (اختياري)')}</Label><Input type="datetime-local" min={spaWindowEditor.bookingStartAt || undefined} value={spaWindowEditor.bookingEndAt} onChange={(event) => setSpaWindowEditor({ ...spaWindowEditor, bookingEndAt: event.target.value })} /></div></div><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setSpaWindowEditor(null)}>{ui('إلغاء')}</Button><Button disabled={savingSpaWindow} onClick={() => void saveSpaAdd()}>{savingSpaWindow ? ui('جارٍ الحفظ...') : ui('إضافة الموعد')}</Button></div></div>}</DialogContent></Dialog>
    <Dialog open={editSlot != null} onOpenChange={(open) => !open && setEditSlot(null)}><DialogContent size="form" aria-describedby={undefined}><DialogHeader><DialogTitle>{ui('تعديل الموعد')}</DialogTitle></DialogHeader><div className="grid gap-4 md:grid-cols-2"><div><Label>{ui('التاريخ')}</Label><Input type="date" value={editForm.slotDate} onChange={(event) => setEditForm({ ...editForm, slotDate: event.target.value })} /></div>{category !== 'spa' && <div><Label>{ui('السعة')}</Label><Input type="number" min="1" value={editForm.capacity} onChange={(event) => setEditForm({ ...editForm, capacity: event.target.value })} /></div>}<div><Label>{ui('بداية الموعد')}</Label><Input type="time" value={editForm.startTime} onChange={(event) => setEditForm({ ...editForm, startTime: event.target.value })} /></div><div><Label>{ui('نهاية الموعد')}</Label><Input type="time" value={editForm.endTime} onChange={(event) => setEditForm({ ...editForm, endTime: event.target.value })} /></div><div><Label>{ui('فتح الحجز')}</Label><Input type="datetime-local" value={editForm.bookingStartAt} onChange={(event) => setEditForm({ ...editForm, bookingStartAt: event.target.value })} /></div><div><Label>{ui('غلق الحجز')}</Label><Input type="datetime-local" value={editForm.bookingEndAt} onChange={(event) => setEditForm({ ...editForm, bookingEndAt: event.target.value })} /></div></div><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setEditSlot(null)}>{ui('إلغاء')}</Button><Button onClick={() => void saveEdit()}>{ui('حفظ التعديل')}</Button></div></DialogContent></Dialog>
