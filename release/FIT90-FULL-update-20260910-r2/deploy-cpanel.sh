...
# It does not restart PM2 and does not change the database.
set -euo pipefail
...
if find "$SOURCE_DIR" -type f -name '.env*' -print -quit | grep -q .; then
...
# A prior root-owned build makes Nest fail while deleting dist/. This must be
# corrected by ROOT-PRECHECK-COMMAND.txt before this non-root deploy starts.
if [ -d "$APP_DIR/dist" ] && [ ! -w "$APP_DIR/dist" ]; then
  echo "ERROR: $APP_DIR/dist is not writable by the cPanel account."
  echo "Ask root to run ROOT-PRECHECK-COMMAND.txt, then rerun this command."
  exit 1
fi
...
echo "==> [2/4] Copying backend source (.env, uploads, node_modules, public, and database are untouched)"
...
# Nest currently emits the PM2 entry point here. Never publish or restart a
# release if the compiled file is absent.
if [ ! -f "$APP_DIR/dist/src/main.js" ]; then
  echo "ERROR: Backend build did not create $APP_DIR/dist/src/main.js. PM2 must not be restarted."
  exit 1
fi
