...
export function BookingsPage({
  ...
  const exportXlsx = () => {
    ...
    if (serviceId != null) qs.set('serviceId', String(serviceId));
    window.open(`/api/scheduling/bookings/export?${qs.toString()}`, '_blank');
  };
      ...
      <div className={`flex flex-wrap items-start gap-3 ${embedded ? 'justify-end' : 'justify-between'}`}>
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={exportXlsx}>
            <Download className="me-1 h-4 w-4" /> {ft('sched.export')}
          </Button>
