      ...
      <PageHeader
        ...
        actions={
          <Badge variant="secondary" className="text-xs">
            {ui('إجمالي خانات الصلاحيات:')} {ar(rolesQ.data?.totalSlots ?? 0)}
          </Badge>
        }
      />
          ...
          <Card className="overflow-hidden">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border bg-muted/30 px-4 py-2">
              <p className="text-xs text-muted-foreground">
                {ui('انقر الخانة للتبديل: وراثة ← سماح ← رفض.')} {dirty && <span className="font-semibold text-primary">({ar(changes.length)} {ui('تغيير غير محفوظ')})</span>}
              </p>
