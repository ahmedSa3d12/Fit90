...
interface StaffNotificationContent extends PushPayload {
  url?: string;
}
...
export class PushService {
  ...
  async sendToUsers(
    ...
    const message = JSON.stringify({ title, body, url } satisfies StaffNotificationContent);
    ...
    for (const userId of userIds) {
      ...
      // 2) carry the actual text (tbl_notifications has no title/body columns)
