...
export class NutritionBookingsService {
  ...
  async createPersonalTrainingAttendance(dto: CreatePersonalTrainingBookingDto, user?: JwtUser) {
    return this.prisma.$transaction(async (tx: any) => {
      ...
      if (existingCount >= Math.max(1, Number(window.capacity ?? 1))) {
        throw new ConflictException('This personal-training attendance is fully booked.');
      }
      ...
      if (!eligibility.hasPersonalTrainingSubscription || eligibility.remaining <= 0) {
        throw new BadRequestException('Member does not have an available personal-training subscription.');
      }
