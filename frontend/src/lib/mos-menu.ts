...
export interface MosMenuItem {
  ...
  icon?: string;
  path?: string;
  children?: MosMenuItem[];
}

export const MOS_MENU: MosMenuItem[] = [
  {
    key: 'dashboard',
    icon: 'dashboard',
    path: '/dashboard',
...
// so its former children do not leak back into the top-level sidebar.
const additionalMenu = MOS_MENU.find((item) => item.key === 'additionalManagement');
const additionalNestedKeys = ['accountsManagement', 'dataList'];
const additionalNestedMenus = additionalNestedKeys
  .map((key) => MOS_MENU.find((item) => item.key === key))
  .filter((item): item is MosMenuItem => Boolean(item));

for (const menu of additionalNestedMenus) {
  const index = MOS_MENU.indexOf(menu);
  if (index !== -1) MOS_MENU.splice(index, 1);
}

if (additionalMenu?.children) {
  additionalMenu.children.unshift(...additionalNestedMenus);
  const additionalIndex = MOS_MENU.indexOf(additionalMenu);
  if (additionalIndex !== -1) MOS_MENU.splice(additionalIndex, 1);
}

// Keep reports directly after personal training in the top-level sidebar.
const reportsIndex = MOS_MENU.findIndex((item) => item.key === 'reports');
const personalTrainingIndex = MOS_MENU.findIndex(
  (item) => item.key === 'personalTrainingManagement',
);

if (reportsIndex !== -1 && personalTrainingIndex !== -1) {
  const [reportsMenu] = MOS_MENU.splice(reportsIndex, 1);
  const updatedPersonalTrainingIndex = MOS_MENU.findIndex(
    (item) => item.key === 'personalTrainingManagement',
  );
  MOS_MENU.splice(updatedPersonalTrainingIndex + 1, 0, reportsMenu);
}
...
const updatedReportsIndex = MOS_MENU.findIndex((item) => item.key === 'reports');
if (updatedReportsIndex !== -1) {
  MOS_MENU.splice(updatedReportsIndex + 1, 0, ...administrationMenus);
}
