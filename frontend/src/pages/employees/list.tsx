...
const STATUS_OPTIONS = [
  { value: '1', label: uiStatic('نشط') },
  { value: '2', label: uiStatic('موقوف') },
  { value: '0', label: uiStatic('لم يحدد') },
];
...
export function EmployeesListPage() {
  ...
  const filters: FilterField[] = [
    { key: 'status', label: ui('الحالة'), type: 'select', options: STATUS_OPTIONS },
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem asChild>
                <Link to={`/employees/${row.original.id}`}>
                  <User className="size-4" /> {ui('الملف')}
                </Link>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
  ...
  if (isError && isNotImplemented(error)) {
    return (
      <div>
        <PageHeader title={ui('الموظفين')} actions={<Button asChild><Link to="/employees/new"><Plus />{ui('موظف جديد')}</Link></Button>} />
  ...
  return (
    <div>
      <PageHeader
        title={ui('الموظفين')}
        description={ui('إدارة بيانات الموظفين — القائمة النشطة')}
        actions={
