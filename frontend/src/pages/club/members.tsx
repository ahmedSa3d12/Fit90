...
import { potentialLeadToMemberPrefill, type PotentialLeadForMember } from './member-potential-prefill';
...
export function ClubMembersPage({ singleView }: ClubMembersPageProps = {}) {
  ...
  const [potentialLeadId, setPotentialLeadId] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(() => {
      void (async () => {
        try {
          const [duplicatesResponse, potentialResponse] = await Promise.all([
            api.get<{ duplicates?: DuplicateHit[] }>('/club-members/check-duplicate', { params: { phone } }),
            api.get<PotentialLeadForMember | null>('/club-members/potential-lead', { params: { phone } }),
          ]);
          if (!cancelled) {
            setDupHits(duplicatesResponse.data.duplicates ?? []);
            setPotentialLeadId(potentialResponse.data?.id ?? null);
            if (potentialResponse.data) {
              const prefill = potentialLeadToMemberPrefill(potentialResponse.data);
              setForm((current) => ({
                ...current,
                ...prefill,
                branchId: prefill.branchId ?? current.branchId,
                phone: current.phone,
              }));
            }
          }
  ...
  const saveMember = async (andAddSubscription = false) => {
    ...
    try {
      ...
      if (editId) {
        ...
        const createPath = potentialLeadId
          ? `/club-members/from-potential/${potentialLeadId}`
          : '/club-members';
        const { data: created } = await api.post<ClubMemberCreateResponse>(createPath, payload);
      ...
      void qc.invalidateQueries({ queryKey: ['club-members'] });
      void qc.invalidateQueries({ queryKey: ['club-mos/entities/leads'] });
    } catch (e) {
