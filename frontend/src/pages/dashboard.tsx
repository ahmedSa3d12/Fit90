...
const QUICK_LINK_DEFS = [
  { to: '/attendance', labelKey: uiStatic('لوحة الحضور'), icon: Fingerprint },
  { to: '/employees', labelKey: uiStatic('الموظفين'), icon: Users },
  { to: '/leaves', labelKey: uiStatic('الإجازات'), icon: CalendarOff },
  { to: '/reports', labelKey: uiStatic('التقارير'), icon: FileText },
] as const;
...
function AttendanceHeroCard({
  ...
  if (isLoading) {
    return (
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#1E6BA8] via-[#155891] to-[#0c3968] p-6 shadow-xl">
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#1E6BA8] via-[#155891] to-[#0c3968] p-6 shadow-xl ring-1 ring-white/10">
      ...
      <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:gap-6">
        ...
        <div className="relative mx-auto shrink-0 sm:mx-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
            <span className="text-2xl font-bold text-white nums">{toArabicDigits(Math.round(attendancePct))}%</span>
...
function DashboardBody({
  ...
  const chartData = useMemo(
    () => data?.weekly?.map((w) => ({ name: w.label, pct: w.pct })) ?? [],
    [data?.weekly],
  );
      ...
      <div className="grid gap-6 xl:grid-cols-[1.7fr_1fr]">
        ...
        <div className="min-w-0 space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <StatCard title={uiStatic('حاضر اليوم')} value={data?.presentToday ?? 0} subtitle={data ? `${toArabicDigits(data.presentPct.toFixed(1))}%` : undefined} icon={<UserCheck className="size-5" />} colorIndex={3} loading={isLoading} />
            <StatCard title={uiStatic('متأخر اليوم')} value={data?.lateToday ?? 0} subtitle={data?.avgLateMinutes ? uiStatic(`متوسط ${toArabicDigits(Math.round(data.avgLateMinutes))} د`) : undefined} icon={<Clock className="size-5" />} colorIndex={4} loading={isLoading} />
          ...
          >
            <AreaChart data={chartData}>
              ...
              <YAxis tickFormatter={(v) => toArabicDigits(v)} domain={[0, 100]} />
              <Tooltip formatter={(v: number) => [`${toArabicDigits(v.toFixed(1))}%`, uiStatic('النسبة')]} />
              <Area type="monotone" dataKey="pct" stroke="#38BDF8" fill="url(#brandFill)" strokeWidth={3} />
            </AreaChart>
          </ChartCard>
                    ...
                    <tbody>
                      {data.todayAttendance.map((row, i) => (
                        <tr key={row.hodoorId} className="border-b border-border/50 transition-colors even:bg-muted/20 hover:bg-primary/5">
                          <td className="p-3 nums">{toArabicDigits(i + 1)}</td>
                          <td className="p-3 nums">{toArabicDigits(row.empCode ?? '—')}</td>
                          <td className="p-3 font-medium">{row.name ?? '—'}</td>
                          <td className="p-3">{row.department ?? '—'}</td>
                          <td className="p-3 nums">{row.checkIn ? toArabicDigits(row.checkIn) : '—'}</td>
                          <td className="p-3 nums">{row.checkOut ? toArabicDigits(row.checkOut) : '—'}</td>
                          <td className="p-3 nums">{row.lateMin != null ? toArabicDigits(row.lateMin) : '—'}</td>
                          <td className="p-3">
                ...
                <>
                  {(data?.lateToday ?? 0) > 0 && (
                    <div className="rounded-xl border border-warning/30 bg-warning/5 p-3.5 text-sm">
                      ...
                      {toArabicDigits(data!.lateToday)} {uiStatic('موظف متأخر اليوم')}
                      {data!.avgLateMinutes > 0 && (
                        <span className="text-muted-foreground">
                          {' '}
                          ({uiStatic(`متوسط ${toArabicDigits(Math.round(data!.avgLateMinutes))} دقيقة`)})
                        </span>
                      )}
                    </div>
                  )}
                  {absentToday > 0 && (
                    <div className="rounded-xl border border-destructive/25 bg-destructive/5 p-3.5 text-sm">
                      ...
                      {toArabicDigits(absentToday)} {uiStatic('موظف لم يسجّل حضوراً اليوم')}
                    </div>
                  )}
                  {(data?.presentPct ?? 0) < 50 && (data?.totalEmployees ?? 0) > 0 && (
                    <div className="rounded-xl border border-primary/25 bg-primary/5 p-3.5 text-sm">
                      {uiStatic(`نسبة الحضور منخفضة (${toArabicDigits(data!.presentPct.toFixed(1))}%) — راجع لوحة الحضور`)}
                    </div>
                  )}
                  ...
                  {alertHighlights.map((a) => (
                    ...
                    >
                      ...
                      <span className="text-muted-foreground">
                        {' '}
                        ({uiStatic('باقي')} <span className="nums">{toArabicDigits(a.daysLeft)}</span> {uiStatic('يوم')})
                      </span>
                    </Link>
                  ))}
                  ...
                  {alertHighlights.length > 0 && (
                    <Button variant="ghost" size="sm" className="w-full" asChild>
                      <Link to="/club/subscriptions">{uiStatic('متابعة التجديدات والاشتراكات')}</Link>
                    </Button>
                  )}
                </>
              )}
...
export function HrSectionDashboard({ hideQuickLinks = true }: { hideQuickLinks?: boolean }) {
  ...
  const branchOptions = useMemo(
    () => branches?.map((b) => ({ value: String(b.id), label: b.name ?? `فرع ${b.id}` })) ?? [],
    [branches],
  );
