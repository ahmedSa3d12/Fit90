...
const LEGACY_ROUTE: Record<string, { key: string; actions: ActionKey[] }> = {
  ...
  // Membership pages were previously grouped under Extra/Reports. Keep their
  // stored keys stable while displaying them beneath Membership Management.
  '/mos/debts': { key: 'mos.extra.debts', actions: LIST },
  '/mos/reports/blockedMembers': { key: 'mos.reports.blockedMembers', actions: REPORT },
  '/mos/reports/members': { key: 'mos.reports.members', actions: REPORT },
...
function resolveResourceKey(
  ...
  if (legacy) {
    if (usedPageKeys.has(legacy.key)) return null;
