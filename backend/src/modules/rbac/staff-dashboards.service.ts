...
import { isNutritionJobTitle, isSalesJobTitle, isSpaJobTitle } from '../employees/trainer-job-title.util';
...
export class StaffDashboardsService {
  ...
  private async providerForEmployee(employee: Awaited<ReturnType<StaffDashboardsService['employeeForUser']>>, fallbackName: string) {
    ...
    if (!provider) {
      provider = await this.prisma.club_trainers.create({
        data: {
          employee_id: employee.id,
          name: employee.employee ?? fallbackName,
  ...
  async nutritionDashboard(userId: number, requestedMonth?: string) {
    return this.providerDashboard(userId, requestedMonth, 'nutrition', isNutritionJobTitle, 'أخصائي تغذية');
  }

  async spaDashboard(userId: number, requestedMonth?: string) {
    return this.providerDashboard(userId, requestedMonth, 'spa', isSpaJobTitle, 'أخصائي سبا');
  }

  private async providerDashboard(
    userId: number,
    requestedMonth: string | undefined,
    category: 'nutrition' | 'spa',
    hasRequiredJobTitle: (value?: string | null) => boolean,
    fallbackName: string,
  ) {
    const month = selectedMonth(requestedMonth);
    const employee = await this.employeeForUser(userId);
    if (!hasRequiredJobTitle(employee.mosma_wazefy_n)) throw new NotFoundException(`الحساب الحالي ليس حساب ${fallbackName}`);
    const provider = await this.providerForEmployee(employee, fallbackName);
    ...
    const providerAppointments = providerWindows.flatMap<NutritionAppointment>((window) => {
      if (!window.nutrition_bookings.length) {
        return [{
          ...
          serviceName: category === 'spa' ? 'سبا' : 'تغذية',
