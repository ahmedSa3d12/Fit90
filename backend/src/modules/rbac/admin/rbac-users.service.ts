...
export class RbacUsersService {
  ...
  async getExceptions(userId: number) {
    ...
    const hasGeneralManagerRole = roleNames.some((role) => role.isSuperAdmin);
    ...
    const [roleAllowSet, exceptions] = await Promise.all([
      hasGeneralManagerRole ? Promise.resolve(this.engine.getAllKeys()) : this.roleOnlyAllowSet(userId),
