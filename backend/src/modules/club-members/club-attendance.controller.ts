...
export class ClubAttendanceController {
  ...
  /** Member check-in always records the current employee as the creator. */
  @Post('my/check-in')
  @RequiresPermission('club.members:create')
  checkInForCurrentEmployee(@Body() body: CheckInDto, @CurrentUser('sub') userId: number) {
    return this.service.checkIn(body, userId);
  }

  @Post('today/check-out')
  @RequiresPermission('club.members:update')
  checkOutToday(@Body() body: CheckOutDto, @CurrentUser('sub') userId: number) {
    if (!body.attendanceId) {
      throw new BadRequestException('رقم سجل الدخول مطلوب');
    }
    return this.service.checkOutToday(body.attendanceId, userId);
  }

  @Get('today')
  listToday(@Query() query: ListClubAttendanceDto) {
    return this.service.listToday(query);
  }
