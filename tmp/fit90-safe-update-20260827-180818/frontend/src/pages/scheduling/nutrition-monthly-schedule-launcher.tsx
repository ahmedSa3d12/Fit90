...
const emptyWindow = () => ({ startTime: '15:00', endTime: '20:00', bookingStartAt: '', bookingEndAt: '' });
...
function NutritionPlanCalendar({ plan, onChanged }: { plan: NutritionMonthlyPlan; onChanged: () => void }) {
  const ui = useUi();
        ...
        <DialogContent size="form" aria-describedby={undefined}>
          ...
          <div className="space-y-4">
            ...
            {windowsForDate(selectedDate).length > 0 && <div className="space-y-2"><b>فترات الحضور المسجلة</b>{windowsForDate(selectedDate).map((window) => <div key={window.id} className="flex items-center justify-between rounded-lg border p-3">
              <span>{window.start_time.slice(0, 5)} — {window.end_time.slice(0, 5)} · {window.bookingCount ?? 0} {ui('حجز')}</span>
...
function NutritionPlanViewDialog({ planId, onClose }: { planId: number | null; onClose: () => void }) {
  const ui = useUi();
  const { data, isLoading, refetch } = useQuery({
...
export function NutritionMonthlyScheduleLauncher() {
  const ui = useUi();
  ...
  const queryKey = ['nutrition-provider-monthly-plans'];
  const { data: plans = [], isLoading } = useQuery({ queryKey, queryFn: async () => (await api.get<NutritionMonthlyPlan[]>('/scheduling/provider-monthly-availability')).data });
  ...
  const create = async () => {
    ...
    try {
      const response = await api.post<NutritionMonthlyPlan>('/scheduling/provider-monthly-availability', { employeeId: Number(form.employeeId), month: Number(form.month), year: Number(form.year) });
  ...
  return <div className="space-y-5">
    <Card><CardHeader><CardTitle>{ui('إنشاء جدول حضور شهري')}</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-4">
      <div><Label>{ui('أخصائي التغذية')}</Label><select className={SELECT_CLS} value={form.employeeId} onChange={(event) => setForm({ ...form, employeeId: event.target.value })}><option value="">{ui(providersLoading ? 'جارٍ تحميل الأخصائيين...' : 'اختر أخصائي التغذية')}</option>{providers.map((provider) => <option key={provider.id} value={provider.id}>{provider.name}</option>)}</select></div>
