...
export function usePermission(): PermissionApi {
  ...
  const canRoute = useCallback(
    (pathname: string) => {
      ...
      // longest matching catalog route wins; unmapped → allowed (utility pages)
      let bestKeys: string[] = [];
      let bestLen = -1;
      ...
      if (!bestKeys.length) return true;
      return bestKeys.some((resourceKey) => keys.has(`${resourceKey}:view`));
    },
    [data, keys, routeMap],
  );
