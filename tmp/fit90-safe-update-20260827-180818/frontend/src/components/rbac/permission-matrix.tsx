...
export function PermissionMatrix({ catalog, mode, draft, onChange, baseline, editable }: PermissionMatrixProps) {
  ...
  useEffect(() => {
    // A promoted club department is still a `group` in the underlying catalog;
    // keep the selected root open so its screens are immediately visible.
    setCollapsed((previous) => {
      if (!previous.has(activeKey)) return previous;
