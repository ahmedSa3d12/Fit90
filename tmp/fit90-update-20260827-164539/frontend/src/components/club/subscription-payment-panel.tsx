...
export function SubscriptionPaymentPanel({
  ...
  const paymentLabel = (method: string | null | undefined) => {
    ...
    const labels: Record<string, string> = {
      cash: ct('payments.cash'),
      card: ct('payments.card'),
      bank: ct('payments.bank'),
      online: ct('payments.online'),
      visa: ct('payments.visa'),
      transfer: ct('payments.transfer'),
      wallet: ct('payments.wallet'),
    };
              ...
              <tbody>
                {receipts.map((r) => (
                  <tr key={r.id} className="border-b last:border-0">
                    ...
                    <td className="p-2">{paymentLabel(r.paymentMethod)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
