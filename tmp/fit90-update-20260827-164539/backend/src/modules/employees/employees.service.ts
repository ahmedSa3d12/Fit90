...
import { isNutritionJobTitle, isTrainerJobTitle } from './trainer-job-title.util';
import {
...
export class EmployeesService {
  ...
  async create(body: Record<string, unknown>, userId?: number) {
    ...
    await this.syncTrainerProfile(created.id);
  ...
  /** Keep the club trainer directory linked to HR employees whose job title is trainer. */
  private async syncTrainerProfile(employeeId: number): Promise<void> {
    ...
    if (!employee || (!isTrainerJobTitle(employee.mosma_wazefy_n) && !isNutritionJobTitle(employee.mosma_wazefy_n))) return;

    const existingTrainer = await this.prisma.club_trainers.findFirst({
    ...
    const data = {
      ...
      is_active: employee.employee_type === 1 && employee.leave_emp !== 1,
      is_deleted: false,
    };
    if (existingTrainer) {
      await this.prisma.club_trainers.update({ where: { id: existingTrainer.id }, data });
    } else {
      await this.prisma.club_trainers.create({ data });
    }
  }
  ...
  async update(id: number, body: Record<string, unknown>, userId?: number) {
    ...
    await this.syncTrainerProfile(id);
  ...
  private async provisionUser(
    ...
    // Orphan cleanup: phone changed → retire old login username.
    if (previousPhone && previousPhone !== phone) {
      const oldUser = await this.prisma.users.findFirst({ where: { username: previousPhone } });
      if (oldUser) {
        await this.prisma.users.update({
          where: { user_id: oldUser.user_id },
          data: { approved: 0, username: `_retired_${oldUser.user_id}_${previousPhone}` },
        });
        await this.prisma.rbac_user_roles.deleteMany({ where: { user_id: oldUser.user_id } });
      }
    }
    ...
    if (existing) {
      if (existing.emp_code !== empId) {
        await this.prisma.users.update({
          where: { user_id: existing.user_id },
          data: { emp_code: empId, name, email: loginEmail, branch_id_fk: branchId, username },
        });
      }
      if (existing.emp_code === empId && existing.email !== loginEmail) {
        await this.prisma.users.update({
          where: { user_id: existing.user_id },
      ...
      if (credentials?.password) {
        ...
        await this.prisma.users.update({
          where: { user_id: existing.user_id },
          data: { password: pwd, app_pass: pwd, pass_demo: pwd, user_pass: pwd, x_y_z: plainPassword },
        });
      }
      if (roleId != null) {
