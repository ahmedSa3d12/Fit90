...
export class TrainerDashboardService {
  ...
  async getDashboard(userId: number, requestedMonth?: string) {
    ...
    const [subscriptions, monthlySalesSubscriptions, schedules, classPlans, legacyClasses] = await Promise.all([
    ...
    const collected = monthlySalesSubscriptions.reduce((sum, item) => sum + Number(item.paid_amount), 0);
    ...
    return {
      ...
      subscriptions: subscriptions.map((subscription) => ({
        ...
        memberCode: subscription.member_id ? memberById.get(subscription.member_id)?.member_code ?? null : null,
        memberName: subscription.member_id ? memberById.get(subscription.member_id)?.name ?? '—' : '—',
        memberPhone: subscription.member_id ? memberById.get(subscription.member_id)?.phone ?? null : null,
      ...
      monthlySalesSubscriptions: monthlySalesSubscriptions.map((subscription) => ({
        id: subscription.id,
        memberName: subscription.member_id
