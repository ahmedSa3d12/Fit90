...
import { CreateNutritionBookingDto } from './dto/nutrition-booking.dto';
...
export class NutritionBookingsService {
  ...
  private async eligibilityWithClient(client: any, memberId: number, service: any, date: string) {
  ...
  async eligibility(memberId: number, serviceId: number, date: string) {
    ...
    return this.eligibilityWithClient(this.prisma, memberId, service, date);
  }
  ...
  async transitionStatus(
    ...
    return this.prisma.$transaction(async (tx: any) => {
      ...
      if (!booking || !booking.availability_slot || !booking.availability_slot.monthly_availability) {
        throw new NotFoundException('Nutrition booking not found.');
      }
      await this.employeeScope.assertProviderAccess(user, booking.employee_id);
      this.assertBranchAccess(user, booking.branch_id);
  ...
  async create(dto: CreateNutritionBookingDto, user?: JwtUser) {
    return this.prisma.$transaction(async (tx: any) => {
      await tx.$queryRaw`
        SELECT p.id
        FROM club_provider_monthly_availabilities p
      ...
      if (!window) throw new NotFoundException('Nutrition provider attendance window not found.');
      ...
      const service = await tx.club_services.findFirst({
        where: { id: dto.serviceId, category: 'nutrition', is_active: true, is_deleted: false },
      });
      if (!service) throw new NotFoundException('Nutrition service not found.');
      ...
      const eligibility = await this.eligibilityWithClient(tx, dto.memberId, service, window.slot_date);
      const schedule = await tx.club_schedules.create({
        data: {
          ...
          booked_count: 1,
          status: 'available',
          notes: `nutrition-availability:${window.id}`,
        },
      });
      const booking = await tx.club_bookings.create({
        data: {
          ...
          entitlement_key_snapshot: service.entitlement_key,
