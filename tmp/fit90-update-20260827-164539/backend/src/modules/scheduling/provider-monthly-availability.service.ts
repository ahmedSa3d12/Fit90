...
type CreateProviderMonthlyAvailability = { employeeId: number; month: number; year: number };
type WindowInput = {
  slotDate: string;
  startTime: string;
  endTime: string;
...
export class ProviderMonthlyAvailabilityService {
  ...
  private assertNutritionSpecialist(provider: { specialization: string | null } | null) {
  ...
  async create(dto: CreateProviderMonthlyAvailability, user?: JwtUser) {
    ...
    if (employeeId == null) throw new NotFoundException('Nutrition specialist not found.');
    const provider = await this.prisma.club_trainers.findFirst({
      where: { id: employeeId, is_active: true, is_deleted: false },
      select: { id: true, specialization: true },
    });
    this.assertNutritionSpecialist(provider);
    ...
    const duplicate = await this.prisma.club_provider_monthly_availabilities.findUnique({
      where: {
        employee_id_category_month_year: {
          employee_id: employeeId, category: 'nutrition', month: dto.month, year: dto.year,
        },
      },
    });
    if (duplicate) throw new ConflictException('A nutrition plan already exists for this specialist and month.');
    const created = await this.prisma.club_provider_monthly_availabilities.create({
      data: { employee_id: employeeId, category: 'nutrition', month: dto.month, year: dto.year },
  ...
  async list(
    query: { employeeId?: number; month?: number; year?: number; status?: 'draft' | 'published' | 'archived' },
    user?: JwtUser,
  ) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const slotDate of dates) {
        const conflict = await tx.club_availability_slots.findFirst({
          where: {
            module_type: 'nutrition', trainer_id: plan.employee_id, slot_date: slotDate,
  ...
  async updateWindow(id: number, windowId: number, dto: WindowInput, user?: JwtUser) {
    return this.prisma.$transaction(async (tx) => {
      ...
      const window = await tx.club_availability_slots.findFirst({
        where: { id: windowId, monthly_availability_id: id, module_type: 'nutrition', is_deleted: false },
      });
      ...
      const conflict = await tx.club_availability_slots.findFirst({
        where: {
          id: { not: windowId },
          module_type: 'nutrition', trainer_id: plan.employee_id, slot_date: dto.slotDate,
          is_deleted: false, start_time: { lt: dto.endTime }, end_time: { gt: dto.startTime },
        },
        select: { id: true },
      });
  ...
  async cancelWindow(id: number, windowId: number, reason: string, user?: JwtUser) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const window = await tx.club_availability_slots.findFirst({
        where: {
          id: windowId, monthly_availability_id: id, module_type: 'nutrition',
