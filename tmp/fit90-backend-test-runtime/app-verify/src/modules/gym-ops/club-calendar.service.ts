...
export class ClubCalendarService {
  ...
  async listEvents(query: { from: string; to: string; branchId?: number; trainerId?: number }) {
  ...
  async moveClass(
    ...
    if (hasConflict) {
      throw new BadRequestException({
        message: 'Schedule conflict — the trainer is already booked in this slot',
        conflicts,
      });
    }
  ...
  async createRecurring(body: {
    ...
    if (!body.weekdays.length) throw new BadRequestException('Select at least one day');
    if (body.weeks < 1 || body.weeks > 52) throw new BadRequestException('Weeks must be between 1 and 52');
  ...
  async exportMembersCsv(branchId?: number): Promise<string> {
    const rows = await this.prisma.club_members.findMany({
      where: { is_deleted: false, ...(branchId ? { branch_id: branchId } : {}) },
      orderBy: { id: 'asc' },
      select: {
        member_code: true,
        name: true,
    ...
    const header = 'member_code,name,phone,email,gender,card_number,branch_id,is_active,created_at';
  ...
  async exportSubscriptionsCsv(branchId?: number): Promise<string> {
