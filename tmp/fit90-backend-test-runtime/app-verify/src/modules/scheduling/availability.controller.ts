...
export class AvailabilityController {
  ...
  providerTimes(
    ...
    @Query('trainerId') trainerId: string,
    @Query('date') date: string,
    @Query('category') category: 'nutrition' | 'spa' | 'personal_training',
    @Query('branchId') branchId?: string,
  ) {
    return this.service.providerAvailableTimes(
      parseInt(serviceId, 10),
      parseInt(trainerId, 10),
      date,
      category === 'spa' ? 'spa' : category === 'personal_training' ? 'personal_training' : 'nutrition',
      branchId == null || branchId === '' ? undefined : parseInt(branchId, 10),
    );
  }
  @Post()
